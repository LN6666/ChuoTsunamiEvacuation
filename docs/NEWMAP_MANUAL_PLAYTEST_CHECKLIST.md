...
- Start the NewMap visual-fix temp player.
- Verify mouse look works in Tourism Mode without holding RMB.
- Verify mouse look works in Evacuation Mode without holding RMB.
- Verify Escape pause unlocks cursor and Resume restores mouse look.
- Verify Start Menu to gameplay transition restores camera control.
- Verify clear day brightness is playable.
- Verify night mode is darker but readable.
...
- Confirm current LOD1/LOD2 visual limitation is acceptable or explicitly rejected.
- Confirm player stands on or very near the visible ground.
