...
$logPath = Join-Path $logsDir "p7c-build-high-detail-scene-unity.log"
...
function Assert-ProtectedPathsUnchanged {
    ...
    if ($protectedChanges.Count -gt 0) {
        Write-Host "P7-C high-detail scene build detected protected path changes:" -ForegroundColor Red
        $protectedChanges | ForEach-Object { Write-Host $_ }
        exit 1
    }
}

Write-Host "P7-C high-detail scene build: starting"
Write-Host "Project: $repoRoot"

$unity = Find-UnityExecutable
if (-not $unity) {
    Write-Host "Unity executable was not found. P7-C high-detail scene build requires Unity Editor invocation." -ForegroundColor Red
...
$arguments += @(
    ...
    "-executeMethod", "P7CHighDetailSceneBuilder.BuildFromCommandLine",
    "-p7cBuildHighDetailSceneQuit",
    "-logFile", $logPath
)
...
while (-not $process.WaitForExit(2000)) {
    if ((Get-Date) -ge $deadline) {
        ...
        Write-Host "P7-C high-detail scene build timed out after $TimeoutSeconds seconds." -ForegroundColor Red
        Write-Host "Log: $logPath"
        exit 1
    }
}
...
if ($process.ExitCode -ne 0) {
    Write-Host "P7-C high-detail scene build failed with Unity exit code $($process.ExitCode)." -ForegroundColor Red
    Write-Host "Log: $logPath"
...
Write-Host "P7-C high-detail scene build: PASS"
exit 0
