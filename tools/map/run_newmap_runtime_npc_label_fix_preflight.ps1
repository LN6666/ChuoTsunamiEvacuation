...
Add-Check "Runtime NPC/label build method exists" ([bool](Select-String -LiteralPath $setupPath -Pattern "BuildNewMapRuntimeNpcLabelPlayerCommandLine" -SimpleMatch -Quiet)) "NewMapSceneSetupUtility"

Invoke-ToolCheck "Runtime/NPC/label JSON validation" "tools\map\validate_newmap_runtime_npc_label_fix_json.ps1"
Invoke-ToolCheck "Concave MeshCollider trigger validation" "tools\map\check_newmap_concave_mesh_triggers.ps1"
Invoke-ToolCheck "NPC lifecycle deadlock validation" "tools\map\check_newmap_npc_lifecycle_deadlock.ps1"
Invoke-ToolCheck "Building/road label coverage validation" "tools\map\check_newmap_building_road_label_coverage.ps1"
...
Add-Check "Concave trigger cleanup diagnostics logged" ($bootstrap -match "concaveMeshTriggerOffenders" -and $bootstrap -match "NeutralizeExistingConcaveMeshTrigger") "NewMapRuntimeBootstrap"
Add-Check "NPC lifecycle smoke logged" ($bootstrap -match "RunNpcLifecycleDiagnosticSmoke" -and $bootstrap -match "NewMap NPC lifecycle smoke") "NewMapRuntimeBootstrap"
Add-Check "NPC contact is local" ($npc -match "ResolveNpcTargetAvoidingPlayerContact" -and $npc -match "collisionWithPlayerDoesNotGlobalPause" -and $npc -match "PreventAllStopDeadlock") "NewMapNpcCrowdPrototype"
...
if (Test-Path -LiteralPath $readinessPath -PathType Leaf) {
    ...
    $allowed = @("ready_for_manual_playtest", "ready_with_documented_npc_or_label_limitations", "ready_with_documented_label_or_collision_limitations", "needs_quick_fix_before_manual_test", "blocked")
...
Write-Host "NewMap runtime/NPC/label fix preflight"
