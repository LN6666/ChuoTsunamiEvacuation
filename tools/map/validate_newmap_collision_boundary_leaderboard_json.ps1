...
Require-Condition ([double]$boundary.radiusMeters -eq 3500.0) "Circular boundary radius must be 3500m."
...
Require-Condition ([double]$tsunami.tsunamiWarningDurationSeconds -eq 300.0) "Tsunami warning duration must remain 300s."
...
Require-Condition ([double]$stamina.staminaMultiplier -eq 200.0) "Final stamina multiplier must remain 200."
Require-Condition ([double]$stamina.sprintSpeedMultiplierAdditional -eq 1.35) "Sprint additional multiplier must remain 1.35."
