...
$manual = Join-Path $root "Assets\Data\P10\newmap_manual_playtest_readiness.json"
...
Add-Check "Building collision precision build method exists" ([bool](Select-String -LiteralPath $setup -Pattern "BuildNewMapBuildingCollisionPrecisionPlayerCommandLine" -SimpleMatch -Quiet)) "NewMapSceneSetupUtility"
Add-Check "No stale scene target" (-not [bool](Select-String -LiteralPath $setup -Pattern "Chuo_GroundRoad_Import_Source" -SimpleMatch -Quiet)) "Build scene target remains Chuo_BaseMap"

Invoke-ToolCheck "Precision JSON validation" "tools\map\validate_newmap_building_collision_precision_json.ps1"
Invoke-ToolCheck "Precision source validation" "tools\map\check_newmap_building_collision_precision.ps1"
Invoke-ToolCheck "Walkable corridor validation" "tools\map\check_newmap_walkable_corridors.ps1"
Invoke-ToolCheck "Collision whitelist validation" "tools\map\check_newmap_collision_whitelist.ps1"
Invoke-ToolCheck "Circular boundary validation" "tools\map\check_newmap_circular_boundary.ps1"
Invoke-ToolCheck "R leaderboard validation" "tools\map\check_newmap_r_leaderboard_toggle.ps1"
...
if (Test-Path -LiteralPath $manual -PathType Leaf) {
    ...
    Add-Check "Manual readiness decision exists" $false $manual
}
...
Write-Host "NewMap building collision precision preflight"
