...
$LogPath = Join-Path $ProjectRoot "Logs\newmap_hardening_temp_build.log"
$BuildPath = "D:\UnityProjects\ChuoTsunamiEvacuation-Builds\NewMapHardeningPre\ChuoTsunamiEvacuation_NewMapHardeningPre.exe"
...
$arguments = @(
    ...
    "-executeMethod", "NewMapSceneSetupUtility.BuildNewMapHardeningTempPlayerCommandLine",
    "-logFile", $LogPath
)

Write-Host "Starting hardening temporary NewMap player build. Log: $LogPath"
...
$result = [ordered]@{
    ...
    note = "Temporary hardening build only. This is not a final release package."
}
...
Write-Host "[PASS] Hardening temporary NewMap player build completed: $BuildPath"
exit 0
