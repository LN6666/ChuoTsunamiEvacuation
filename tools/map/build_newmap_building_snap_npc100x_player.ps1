...
$LogPath = Join-Path $ProjectRoot "Logs\newmap_building_snap_npc100x_build.log"
$BuildPath = "D:\UnityProjects\ChuoTsunamiEvacuation-Builds\NewMapBuildingSnapNpc100xPre\ChuoTsunamiEvacuation_NewMapBuildingSnapNpc100xPre.exe"
...
$arguments = @(
    ...
    "-executeMethod", "NewMapSceneSetupUtility.BuildNewMapBuildingSnapNpc100xPlayerCommandLine",
    "-logFile", $LogPath
)

Write-Host "Starting Building Snap + NPC100x temporary NewMap player build. Log: $LogPath"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    $playerDataConfigDir = Join-Path (Join-Path (Split-Path -Parent $BuildPath) "ChuoTsunamiEvacuation_NewMapBuildingSnapNpc100xPre_Data") "Data\P10"
    New-Item -ItemType Directory -Force -Path $playerDataConfigDir | Out-Null
    $configNames = @(
        "newmap_npc_distribution_config.json",
        "newmap_floating_building_snapdown_config.json",
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    Write-Host "Launching Building Snap + NPC100x player smoke with -newmapSelfAuditSmoke for $SmokeDurationSeconds seconds."
...
$result = [ordered]@{
    ...
    note = "Temporary Building Snap + NPC100x player only. This is not a final release package."
    playerLogSummary = "pending"
    buildingSnapdownDiagnostics = "pending_player_log_parse"
    npc100xDistributionDiagnostics = "pending_player_log_parse"
    groundCoverRegressionDiagnostics = "pending_player_log_parse"
    blueAreaDiagnostics = "pending_player_log_parse"
    fallOutPreventionDiagnostics = "pending_player_log_parse"
...
Write-Host "[PASS] Building Snap + NPC100x temporary NewMap player build and launch smoke completed: $BuildPath"
exit 0
