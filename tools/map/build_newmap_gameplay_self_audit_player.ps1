...
$LogPath = Join-Path $ProjectRoot "Logs\newmap_gameplay_self_audit_build.log"
...
$arguments = @(
    ...
    "-executeMethod", "NewMapSceneSetupUtility.BuildNewMapGameplaySelfAuditPlayerCommandLine",
    "-logFile", $LogPath
)

Write-Host "Starting gameplay self-audit temporary NewMap player build. Log: $LogPath"
...
$result = [ordered]@{
    ...
    note = "Temporary gameplay self-audit build only. This is not a final release package."
}
...
Write-Host "[PASS] Gameplay self-audit temporary NewMap player build completed: $BuildPath"
exit 0
