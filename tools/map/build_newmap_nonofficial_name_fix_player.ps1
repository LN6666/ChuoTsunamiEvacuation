...
$LogPath = Join-Path $ProjectRoot "Logs\newmap_nonofficial_name_fix_build.log"
$BuildPath = "D:\UnityProjects\ChuoTsunamiEvacuation-Builds\NewMapNonOfficialNameFixPre\ChuoTsunamiEvacuation_NewMapNonOfficialNameFixPre.exe"
...
$arguments = @(
    ...
    "-executeMethod", "NewMapSceneSetupUtility.BuildNewMapNonOfficialNameFixPlayerCommandLine",
    "-logFile", $LogPath
)

Write-Host "Starting non-official name-fix temporary NewMap player build. Log: $LogPath"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    $playerDataConfigDir = Join-Path (Join-Path (Split-Path -Parent $BuildPath) "ChuoTsunamiEvacuation_NewMapNonOfficialNameFixPre_Data") "Data\P10"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    Write-Host "Launching non-official name-fix player smoke with -newmapSelfAuditSmoke for $SmokeDurationSeconds seconds."
...
$result = [ordered]@{
    ...
    note = "Temporary non-official name-fix player only. This is not a final release package."
    playerLogSummary = "pending"
    labelRuntimeSmoke = "pending_player_log_parse"
    nonOfficialCandidateLabelSmoke = "pending_player_log_parse"
    runtimeNoWebCheck = "pending_player_log_parse"
...
Write-Host "[PASS] Non-official name-fix temporary player build and launch smoke completed: $BuildPath"
exit 0
