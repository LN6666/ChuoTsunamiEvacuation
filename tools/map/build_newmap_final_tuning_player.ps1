...
$LogPath = Join-Path $ProjectRoot "Logs\newmap_final_tuning_build.log"
$PlayerLogPath = Join-Path $ProjectRoot "Logs\newmap_final_tuning_player.log"
$BuildPath = "D:\UnityProjects\ChuoTsunamiEvacuation-Builds\NewMapFinalTuningPre\ChuoTsunamiEvacuation_NewMapFinalTuningPre.exe"
$OutputJson = Join-Path $ProjectRoot "Assets\Data\P10\newmap_final_tuning_player_report.json"
...
$arguments = @(
    ...
    "-executeMethod", "NewMapSceneSetupUtility.BuildNewMapFinalTuningPlayerCommandLine",
    "-logFile", $LogPath
)

Write-Host "Starting NewMap final tuning temporary player build. Log: $LogPath"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    $playerDataConfigDir = Join-Path (Join-Path (Split-Path -Parent $BuildPath) "ChuoTsunamiEvacuation_NewMapFinalTuningPre_Data") "Data\P10"
    ...
    $configNames = @(
        ...
        "newmap_building_collision_final_refinement.json",
        ...
        "newmap_spawn_final_safety_config.json",
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    ...
    Write-Host "Launching NewMap final tuning smoke for $SmokeDurationSeconds seconds. Player log: $PlayerLogPath"
...
$result = [ordered]@{
    ...
    note = "Temporary NewMap final tuning player only. This is not a final release package."
...
Write-Host "[PASS] NewMap final tuning temporary player build and launch smoke completed: $BuildPath"
exit 0
