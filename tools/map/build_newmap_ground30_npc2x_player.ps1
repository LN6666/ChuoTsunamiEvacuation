...
$LogPath = Join-Path $ProjectRoot "Logs\newmap_ground30_npc2x_build.log"
$PlayerLogPath = Join-Path $ProjectRoot "Logs\newmap_ground30_npc2x_player.log"
$BuildPath = "D:\UnityProjects\ChuoTsunamiEvacuation-Builds\NewMapGround30Npc2xPre\ChuoTsunamiEvacuation_NewMapGround30Npc2xPre.exe"
...
$arguments = @(
    ...
    "-executeMethod", "NewMapSceneSetupUtility.BuildNewMapGround30Npc2xPlayerCommandLine",
    "-logFile", $LogPath
)

Write-Host "Starting NewMap ground30/NPC2x temporary player build. Log: $LogPath"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    $playerDataConfigDir = Join-Path (Join-Path (Split-Path -Parent $BuildPath) "ChuoTsunamiEvacuation_NewMapGround30Npc2xPre_Data") "Data\P10"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    ...
    Write-Host "Launching NewMap ground30/NPC2x smoke for $SmokeDurationSeconds seconds. Player log: $PlayerLogPath"
...
$result = [ordered]@{
    ...
    note = "Temporary NewMap 30 percent ground raise + 2x 2.27km NPC distribution player only. This is not a final release package."
...
Write-Host "[PASS] NewMap ground30/NPC2x temporary player build and launch smoke completed: $BuildPath"
exit 0
