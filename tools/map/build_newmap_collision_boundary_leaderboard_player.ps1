...
$LogPath = Join-Path $ProjectRoot "Logs\newmap_collision_boundary_leaderboard_build.log"
$PlayerLogPath = Join-Path $ProjectRoot "Logs\newmap_collision_boundary_leaderboard_player.log"
$BuildPath = "D:\UnityProjects\ChuoTsunamiEvacuation-Builds\NewMapCollisionBoundaryLeaderboardPre\ChuoTsunamiEvacuation_NewMapCollisionBoundaryLeaderboardPre.exe"
...
$arguments = @(
    ...
    "-executeMethod", "NewMapSceneSetupUtility.BuildNewMapCollisionBoundaryLeaderboardPlayerCommandLine",
    "-logFile", $LogPath
)

Write-Host "Starting NewMap collision/boundary/leaderboard temporary player build. Log: $LogPath"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    $playerDataConfigDir = Join-Path (Join-Path (Split-Path -Parent $BuildPath) "ChuoTsunamiEvacuation_NewMapCollisionBoundaryLeaderboardPre_Data") "Data\P10"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    ...
    Write-Host "Launching NewMap collision/boundary/leaderboard smoke for $SmokeDurationSeconds seconds. Player log: $PlayerLogPath"
...
$result = [ordered]@{
    ...
    note = "Temporary NewMap collision/boundary/leaderboard player only. This is not a final release package."
...
Write-Host "[PASS] NewMap collision/boundary/leaderboard temporary player build and launch smoke completed: $BuildPath"
exit 0
