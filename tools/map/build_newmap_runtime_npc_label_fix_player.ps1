...
$LogPath = Join-Path $ProjectRoot "Logs\newmap_runtime_npc_label_build.log"
$BuildPath = "D:\UnityProjects\ChuoTsunamiEvacuation-Builds\NewMapRuntimeNpcLabelFixPre\ChuoTsunamiEvacuation_NewMapRuntimeNpcLabelFixPre.exe"
...
$arguments = @(
    ...
    "-executeMethod", "NewMapSceneSetupUtility.BuildNewMapRuntimeNpcLabelPlayerCommandLine",
    "-logFile", $LogPath
)

Write-Host "Starting Runtime/NPC/Label temporary NewMap player build. Log: $LogPath"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    $playerDataConfigDir = Join-Path (Join-Path (Split-Path -Parent $BuildPath) "ChuoTsunamiEvacuation_NewMapRuntimeNpcLabelFixPre_Data") "Data\P10"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    Write-Host "Launching Runtime/NPC/Label player smoke with -newmapSelfAuditSmoke for $SmokeDurationSeconds seconds."
...
$result = [ordered]@{
    ...
    note = "Temporary Runtime/NPC/Label player only. This is not a final release package."
    playerLogSummary = "pending"
    concaveMeshTriggerDiagnostics = "pending_player_log_parse"
    npcLifecycleDiagnostics = "pending_player_log_parse"
    playerNpcContactDiagnostics = "pending_player_log_parse"
    labelRuntimeDiagnostics = "pending_player_log_parse"
    runtimeNoWebDiagnostics = "pending_player_log_parse"
...
Write-Host "[PASS] Runtime/NPC/Label temporary NewMap player build and launch smoke completed: $BuildPath"
exit 0
