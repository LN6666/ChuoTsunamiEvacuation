...
Add-Check "PlayerSpawnRoot exists" ([bool](Select-String -LiteralPath $scenePath -Pattern "m_Name: PlayerSpawnRoot" -SimpleMatch -Quiet)) "m_Name: PlayerSpawnRoot"
...
& (Join-Path $root "tools\map\validate_newmap_remaining_hardening_json.ps1") -ProjectRoot $root
Add-Check "Remaining-hardening JSON validation" ($LASTEXITCODE -eq 0) "validate_newmap_remaining_hardening_json.ps1"
...
& (Join-Path $root "tools\map\check_newmap_non_official_candidate_recovery.ps1") -ProjectRoot $root
Add-Check "Non-official candidate recovery" ($LASTEXITCODE -eq 0) "check_newmap_non_official_candidate_recovery.ps1"
...
if (Test-Path -LiteralPath $activePath -PathType Leaf) {
    ...
    Add-Check "Recovered non-official count not stuck at 4" ([int]$active.activeNonOfficialHumanitarianCandidateCount -gt 4) "activeRecovered=$($active.activeNonOfficialHumanitarianCandidateCount)"
...
$stagedBuilds = @($cached | Where-Object { $_ -match "Builds|NewMapRemainingHardeningPre|\.exe$|\.zip$|\.7z$" })
Add-Check "No build artifacts staged" ($stagedBuilds.Count -eq 0) ($stagedBuilds -join ", ")

Write-Host "NewMap remaining hardening preflight"
