...
$LogPath = Join-Path $ProjectRoot "Logs\newmap_ground_raise_name_build.log"
$BuildPath = "D:\UnityProjects\ChuoTsunamiEvacuation-Builds\NewMapGroundRaiseNamePre\ChuoTsunamiEvacuation_NewMapGroundRaiseNamePre.exe"
...
$arguments = @(
    ...
    "-executeMethod", "NewMapSceneSetupUtility.BuildNewMapGroundRaiseNamePlayerCommandLine",
    "-logFile", $LogPath
)

Write-Host "Starting Ground Raise + Name temporary NewMap player build. Log: $LogPath"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    $playerDataConfigDir = Join-Path (Join-Path (Split-Path -Parent $BuildPath) "ChuoTsunamiEvacuation_NewMapGroundRaiseNamePre_Data") "Data\P10"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    Write-Host "Launching Ground Raise + Name player smoke with -newmapSelfAuditSmoke for $SmokeDurationSeconds seconds."
...
$result = [ordered]@{
    ...
    note = "Temporary Ground Raise + Name player only. This is not a final release package."
    playerLogSummary = "pending"
    groundRaiseDiagnostics = "pending_player_log_parse"
    playerNpcTargetHeightDiagnostics = "pending_player_log_parse"
    playerBuildingCollisionDiagnostics = "pending_player_log_parse"
    npcMovementDiagnostics = "pending_player_log_parse"
...
Write-Host "[PASS] Ground Raise + Name temporary NewMap player build and launch smoke completed: $BuildPath"
exit 0
