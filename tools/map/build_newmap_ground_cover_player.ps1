...
$LogPath = Join-Path $ProjectRoot "Logs\newmap_ground_cover_build.log"
$BuildPath = "D:\UnityProjects\ChuoTsunamiEvacuation-Builds\NewMapGroundCoverPre\ChuoTsunamiEvacuation_NewMapGroundCoverPre.exe"
...
$arguments = @(
    ...
    "-executeMethod", "NewMapSceneSetupUtility.BuildNewMapGroundCoverPlayerCommandLine",
    "-logFile", $LogPath
)

Write-Host "Starting Ground Cover temporary NewMap player build. Log: $LogPath"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    $playerDataConfigDir = Join-Path (Join-Path (Split-Path -Parent $BuildPath) "ChuoTsunamiEvacuation_NewMapGroundCoverPre_Data") "Data\P10"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    Write-Host "Launching Ground Cover player smoke with -newmapSelfAuditSmoke for $SmokeDurationSeconds seconds."
...
$result = [ordered]@{
    ...
    groundCoverDiagnostics = "pending_player_log_parse"
    ...
    npcGroundingDiagnostics = "pending_player_log_parse"
    ...
    note = "Temporary Ground Cover player only. This is not a final release package."
}
...
Write-Host "[PASS] Ground Cover temporary NewMap player build and launch smoke completed: $BuildPath"
exit 0
