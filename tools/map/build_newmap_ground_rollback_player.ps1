...
$LogPath = Join-Path $ProjectRoot "Logs\newmap_ground_rollback_build.log"
$BuildPath = "D:\UnityProjects\ChuoTsunamiEvacuation-Builds\NewMapGroundRollbackPre\ChuoTsunamiEvacuation_NewMapGroundRollbackPre.exe"
...
$arguments = @(
    ...
    "-executeMethod", "NewMapSceneSetupUtility.BuildNewMapGroundRollbackPlayerCommandLine",
    "-logFile", $LogPath
)

Write-Host "Starting Ground Rollback temporary NewMap player build. Log: $LogPath"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    $playerDataConfigDir = Join-Path (Join-Path (Split-Path -Parent $BuildPath) "ChuoTsunamiEvacuation_NewMapGroundRollbackPre_Data") "Data\P10"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    Write-Host "Launching Ground Rollback player smoke with -newmapSelfAuditSmoke for $SmokeDurationSeconds seconds."
...
$result = [ordered]@{
    ...
    safeGroundDiagnostics = "pending_player_log_parse"
    ...
    note = "Temporary Ground Rollback player only. This is not a final release package."
}
...
Write-Host "[PASS] Ground Rollback temporary NewMap player build and launch smoke completed: $BuildPath"
exit 0
