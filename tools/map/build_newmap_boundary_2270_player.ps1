...
$LogPath = Join-Path $ProjectRoot "Logs\newmap_boundary_2270_build.log"
$PlayerLogPath = Join-Path $ProjectRoot "Logs\newmap_boundary_2270_player.log"
$BuildPath = "D:\UnityProjects\ChuoTsunamiEvacuation-Builds\NewMapBoundary2270Pre\ChuoTsunamiEvacuation_NewMapBoundary2270Pre.exe"
...
$arguments = @(
    ...
    "-executeMethod", "NewMapSceneSetupUtility.BuildNewMapBoundary2270PlayerCommandLine",
    "-logFile", $LogPath
)

Write-Host "Starting NewMap 2.27km boundary temporary player build. Log: $LogPath"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    $playerDataConfigDir = Join-Path (Join-Path (Split-Path -Parent $BuildPath) "ChuoTsunamiEvacuation_NewMapBoundary2270Pre_Data") "Data\P10"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    ...
    Write-Host "Launching NewMap 2.27km boundary smoke for $SmokeDurationSeconds seconds. Player log: $PlayerLogPath"
...
$result = [ordered]@{
    ...
    note = "Temporary NewMap 2.27km boundary player only. This is not a final release package."
...
Write-Host "[PASS] NewMap 2.27km boundary temporary player build and launch smoke completed: $BuildPath"
exit 0
