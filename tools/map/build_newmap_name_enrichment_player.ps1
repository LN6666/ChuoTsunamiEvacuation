...
$LogPath = Join-Path $ProjectRoot "Logs\newmap_name_enrichment_build.log"
$BuildPath = "D:\UnityProjects\ChuoTsunamiEvacuation-Builds\NewMapNameEnrichmentPre\ChuoTsunamiEvacuation_NewMapNameEnrichmentPre.exe"
...
$arguments = @(
    ...
    "-executeMethod", "NewMapSceneSetupUtility.BuildNewMapNameEnrichmentPlayerCommandLine",
    "-logFile", $LogPath
)

Write-Host "Starting Name Enrichment temporary NewMap player build. Log: $LogPath"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    $playerDataConfigDir = Join-Path (Join-Path (Split-Path -Parent $BuildPath) "ChuoTsunamiEvacuation_NewMapNameEnrichmentPre_Data") "Data\P10"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    Write-Host "Launching Name Enrichment player smoke with -newmapSelfAuditSmoke for $SmokeDurationSeconds seconds."
...
$result = [ordered]@{
    ...
    note = "Temporary Name Enrichment player only. This is not a final release package."
...
Write-Host "[PASS] Name Enrichment temporary NewMap player build and launch smoke completed: $BuildPath"
exit 0
