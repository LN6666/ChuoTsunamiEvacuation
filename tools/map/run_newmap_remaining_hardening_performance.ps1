param(
    ...
    [string]$ProcessName = "ChuoTsunamiEvacuation_NewMapRemainingHardeningPre",
    [string]$BuildPath = "D:\UnityProjects\ChuoTsunamiEvacuation-Builds\NewMapRemainingHardeningPre\ChuoTsunamiEvacuation_NewMapRemainingHardeningPre.exe",
...
if (-not $process) {
    $result = [ordered]@{
        ...
        finalStatus = "failed"
        limitation = "No remaining-hardening player process was running and build path was not found."
    }
    $result | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $output -Encoding UTF8
    Write-Host "[FAIL] No process found for remaining-hardening performance sampling."
    exit 1
}
...
Write-Host "[PASS] Wrote remaining-hardening performance sampling result to $output"
exit 0
