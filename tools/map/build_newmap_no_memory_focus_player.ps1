...
$LogPath = Join-Path $ProjectRoot "Logs\newmap_no_memory_focus_build.log"
...
$arguments = @(
    ...
    "-executeMethod", "NewMapSceneSetupUtility.BuildNewMapNoMemoryFocusPlayerCommandLine",
    "-logFile", $LogPath
)

Write-Host "Starting no-memory-focus temporary NewMap player build. Log: $LogPath"
...
$result = [ordered]@{
    ...
    note = "Temporary no-memory-focus hardening build only. This is not a final release package."
}
...
Write-Host "[PASS] No-memory-focus temporary NewMap player build completed: $BuildPath"
exit 0
