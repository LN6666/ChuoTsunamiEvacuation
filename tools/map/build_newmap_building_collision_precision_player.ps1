...
$LogPath = Join-Path $ProjectRoot "Logs\newmap_building_collision_precision_build.log"
$PlayerLogPath = Join-Path $ProjectRoot "Logs\newmap_building_collision_precision_player.log"
$BuildPath = "D:\UnityProjects\ChuoTsunamiEvacuation-Builds\NewMapBuildingCollisionPrecisionPre\ChuoTsunamiEvacuation_NewMapBuildingCollisionPrecisionPre.exe"
...
$arguments = @(
    ...
    "-executeMethod", "NewMapSceneSetupUtility.BuildNewMapBuildingCollisionPrecisionPlayerCommandLine",
    "-logFile", $LogPath
)

Write-Host "Starting NewMap building-collision-precision temporary player build. Log: $LogPath"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    $playerDataConfigDir = Join-Path (Join-Path (Split-Path -Parent $BuildPath) "ChuoTsunamiEvacuation_NewMapBuildingCollisionPrecisionPre_Data") "Data\P10"
    New-Item -ItemType Directory -Force -Path $playerDataConfigDir | Out-Null
    $configNames = @(
        "newmap_airwall_hard_cleanup_config.json",
        "newmap_building_collision_precision_config.json",
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    ...
    Write-Host "Launching NewMap building-collision-precision smoke for $SmokeDurationSeconds seconds. Player log: $PlayerLogPath"
...
$result = [ordered]@{
    ...
    note = "Temporary NewMap building-collision-precision player only. This is not a final release package."
...
Write-Host "[PASS] NewMap building-collision-precision temporary player build and launch smoke completed: $BuildPath"
exit 0
