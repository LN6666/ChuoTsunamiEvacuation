...
$LogPath = Join-Path $ProjectRoot "Logs\newmap_airwall_npc_label_build.log"
$BuildPath = "D:\UnityProjects\ChuoTsunamiEvacuation-Builds\NewMapAirwallNpcLabelFixPre\ChuoTsunamiEvacuation_NewMapAirwallNpcLabelFixPre.exe"
...
$arguments = @(
    ...
    "-executeMethod", "NewMapSceneSetupUtility.BuildNewMapAirwallNpcLabelPlayerCommandLine",
    "-logFile", $LogPath
)

Write-Host "Starting Airwall/NPC/Label temporary NewMap player build. Log: $LogPath"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    $playerDataConfigDir = Join-Path (Join-Path (Split-Path -Parent $BuildPath) "ChuoTsunamiEvacuation_NewMapAirwallNpcLabelFixPre_Data") "Data\P10"
...
if ($buildProcess.ExitCode -eq 0 -and $exists) {
    Write-Host "Launching Airwall/NPC/Label player smoke with -newmapSelfAuditSmoke for $SmokeDurationSeconds seconds."
...
$result = [ordered]@{
    ...
    note = "Temporary Airwall/NPC/Label player only. This is not a final release package."
    playerLogSummary = "pending"
    airwallDiagnostics = "pending_player_log_parse"
    playerNpcCollisionDiagnostics = "pending_player_log_parse"
...
Write-Host "[PASS] Airwall/NPC/Label temporary NewMap player build and launch smoke completed: $BuildPath"
exit 0
